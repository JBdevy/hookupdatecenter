...
function renderRows(items, type) {
  ...
  return items.map((item, index) => {
    ...
    return `<div class="${classes.join(' ')}" data-item-id="${escapeHtml(itemId)}" data-item-type="${escapeHtml(type)}"${blockOutlineStyle}>${progressBarHtml}${numberCol}<div class="leftCol"><span class="rowLabelText ${textClass}"${labelStyle}>${escapeHtml(label)}</span></div>${rightColHtml}</div>`
  }).join('')
}
