...
function updateBridgeState(data) {
  ...
  // No app dos musicos, a aba e controle local do usuario.
  // O Lua pode trocar entre Repertorios/Musicas sem puxar a tela do app.
  if (!state.activeTab) state.activeTab = 'playlist'
...
function renderGearModal() {
  ...
  return `<div class="modalOverlay" data-close-gear>
    ...
    <div class="modalBox settingsModalBox" data-stop-modal>
      ...
      <div class="settingsSectionTitle">PROJETOS</div>
      <div class="settingsGrid settingsGridSingle">
        <button class="settingsToggleBtn settingsToggleWide" data-action="open-project-tabs">PROJETOS</button>
      </div>
      ...
      <div class="modalButtons">
        ...
        <button class="modalOkBtnWide" data-action="close-gear">FECHAR</button>
      </div>
    </div>
...
function renderNowPlayingLine() {
  ...
  const playbackItem = getPlaybackAwareItem(item, true, false)
  const name = upperText(item?.name || item?.label || '')
  if (!name) return ''
  const ratio = Math.max(0, Math.min(1, getRowProgressRatio(playbackItem, true, false)))
  const percent = Math.round(ratio * 1000) / 10
  return `<div class="musicosNowPlayingLine" data-musicos-now-playing="1" style="display:block;box-sizing:border-box;width:100%;margin:0 0 8px 0;padding:7px 10px 8px;border-radius:12px;border:1px solid rgba(249,115,22,0.55);background:rgba(24,12,6,0.96);color:#ff9a2f;font-weight:800;font-size:14px;line-height:1.15;box-shadow:inset 0 0 0 1px rgba(249,115,22,0.14), 0 0 14px rgba(249,115,22,0.08);">
    <div style="white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${escapeHtml(name)}</div>
    <div class="musicosNowPlayingProgressTrack" style="display:block;width:100%;height:7px;margin-top:7px;border-radius:999px;background:rgba(255,255,255,0.18);overflow:hidden;">
      <div class="musicosNowPlayingProgressFill" data-musicos-now-playing-progress="1" style="display:block;height:100%;width:${percent}%;min-width:${percent > 0 ? '8px' : '0'};border-radius:999px;background:#22c55e;"></div>
    </div>
  </div>`
}
...
function render() {
  ...
  const contentLabel = state.activeTab === 'playlist' ? 'REPERTÓRIOS' : 'MÚSICAS'
  ...
  app.innerHTML = `<div class="app" data-theme="${escapeHtml(state.theme)}">
    ${popupHtml}
    <div class="container" style="${borderStyle}">
      <div class="musicosStickyPanel">
        ...
        <div class="musicosHeaderRow">
          <div class="tabRow">
            <button class="${state.activeTab === 'playlist' ? 'activeTab' : 'tab'}" type="button" data-action="musicos-tab-playlist">REPERTÓRIOS</button>
            <button class="${state.activeTab === 'regions' ? 'activeTab' : 'tab'}" type="button" data-action="musicos-tab-regions">MÚSICAS</button>
            <span class="headerTotal">${escapeHtml(topTime)}</span><span class="musicosHeaderSpacer"></span><button class="tab markersNavButton markersNavButtonWide lyricsNavButton musicosLyricsNavButton" data-action="open-lyrics-panel">&lt;&lt;</button>
          </div>
        </div>
      </div>
      <div class="musicosContentPanel musicosContentWithFooter" style="display:flex;flex-direction:column;min-height:0;flex:1 1 auto;padding-bottom:0;">
        <div class="sectionLabel musicosSectionLabel" style="margin-bottom:6px;">${contentLabel}</div>
        ${nowPlayingHtml}
...
function bindEvents() {
  ...
  document.querySelector('[data-action="musicos-tab-regions"]')?.addEventListener('click', () => setMusicosActiveTab('regions'))
