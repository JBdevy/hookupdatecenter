...
function registerPwaServiceWorker() {
  ...
  try {
    ...
    if (params.get('qr') === '1' || params.get('bridge') === '1' || port === 47831 || port === 47832) return
  } catch (error) {}
...
function closeMixerVolumeModal() {
  if (shouldIgnoreOverlayClose()) return
  state.showMixerVolumeModal = false
  state.mixerVolumeInteracting = false
...
function renderNowPlayingBanner() {
  const label = getNowPlayingLabel()
  if (!label) return ''
  return `<div data-now-playing-banner style="padding:6px 10px 8px 10px;"><div style="min-height:28px;display:flex;align-items:center;padding:0 2px;border-top:1px solid rgba(249,115,22,0.28);color:#f97316;font-weight:800;font-size:15px;letter-spacing:0.02em;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${escapeHtml(label)}</div></div>`
}
...
function bindModalCloseAction(el, actionKey, handler) {
  ...
  el.addEventListener('click', (event) => {
    swallow(event)
    const dedupeKey = `${String(actionKey || '')}:click`
    if (isDuplicatePress(dedupeKey, 260)) return
    window.setTimeout(() => {
      handler(event)
    }, 0)
  })
...
function scheduleScrollSyncCommand(pageKey, ratio) {
  ...
  const targetPage = pageKey === 'markers' ? 'playlist' : pageKey
  const partsVisible = pageKey === 'markers' ? '1' : '0'

  if (pendingScrollSyncFrame) {
    window.cancelAnimationFrame(pendingScrollSyncFrame)
    pendingScrollSyncFrame = 0
  }

  pendingScrollSyncFrame = window.requestAnimationFrame(() => {
    pendingScrollSyncFrame = 0
    lastScrollCommandAt = Date.now()
    postCommand('sync_scroll', {
      page: targetPage,
      partsVisible,
      scrollRatio: safeRatio,
      ratio: safeRatio,
    })
  })
}
...
function applyBridgeScrollToVisibleList() {
  if ((Date.now() - (lastUserScrollAt || 0)) < 4200) return
  const listEl = document.querySelector('.listBox')
  if (!listEl) return
  const pageKey = getActiveScrollPageKey()
  const ratio = getBridgeScrollRatioForPage(pageKey)
  const remoteKey = `${pageKey}|${ratio.toFixed(4)}`
  if (remoteKey === lastAppliedRemoteScrollKey) return
  const maxScroll = Math.max(0, listEl.scrollHeight - listEl.clientHeight)
  const target = maxScroll <= 0 ? 0 : Math.round(maxScroll * ratio)
  if (Math.abs((listEl.scrollTop || 0) - target) <= 2) {
    lastAppliedRemoteScrollKey = remoteKey
    return
  }
  listScrollSyncIgnoreUntil = Date.now() + 420
  lastAppliedRemoteScrollKey = remoteKey
...
function syncFromBridge(data) {
  ...
  if (data.mixer && typeof data.mixer === 'object') {
    const incomingTracks = Array.isArray(data.mixer.tracks) ? mapMixerIncomingItemsWithPending('tracks', data.mixer.tracks) : state.mixerTracks
    const incomingGroups = Array.isArray(data.mixer.groups) ? mapMixerIncomingItemsWithPending('groups', data.mixer.groups) : state.mixerGroups
    const incomingMaster = data.mixer.master && typeof data.mixer.master === 'object'
      ? decorateMixerIncomingItem('master', data.mixer.master)
      : state.mixerMaster
  ...
  if (Number.isFinite(Number(data.regionsScrollRatio))) state.regionsScrollRatio = Math.max(0, Math.min(1, Number(data.regionsScrollRatio)))
  if (Number.isFinite(Number(data.playlistScrollRatio))) state.playlistScrollRatio = Math.max(0, Math.min(1, Number(data.playlistScrollRatio)))
  if (Number.isFinite(Number(data.markersScrollRatio))) state.markersScrollRatio = Math.max(0, Math.min(1, Number(data.markersScrollRatio)))
  ...
  state.queuedSongId = typeof data.queuedSongId === 'string' || typeof data.queuedSongId === 'number' ? String(data.queuedSongId) : null
  state.clearButtonSide = data.clearButtonSide === 'left' ? 'left' : 'right'

  const remoteBaseTab = (data.currentPage === 'playlist' || data.currentPage === 'markers') ? 'playlist' : 'regions'
  if (state.showMixerModal || state.showMixerVolumeModal || state.showBpmModal || state.showTunerModal) {
    return
  }
...
function buildBridgeRenderSignature() {
  return JSON.stringify({
    ...
    playbackTick: (state.playingId && !state.lyricsPanelOpen) ? Math.floor(Date.now() / 200) : 0,
...
async function sendDirectorRecado() {
  ...
  try {
    ...
    state.recadosStatus = 'RECADO ENVIADO POR 10S'
...
function renderRecadosModal() {
  ...
  return `<div class="modalOverlay recadosOverlay" data-close-recados><div class="modalSpacer"></div><div class="modalBox recadosModalBox" data-stop-modal><div class="recadosTopButtons"><button class="modalOkBtnWide recadosSendBtn" data-action="recados-send" ${state.recadosSending ? 'disabled' : ''}>${state.recadosSending ? 'ENVIANDO...' : 'ENVIAR'}</button><button class="modalCancelBtn recadosCancelBtn" data-action="recados-cancel">CANCELAR</button></div><textarea id="recadosDirectorTextarea" class="recadosTextarea" placeholder="Digite o recado técnico..." maxlength="500">${escapeHtml(state.recadosDraft || '')}</textarea>${state.recadosStatus ? `<div class="recadosStatus">${escapeHtml(state.recadosStatus)}</div>` : ''}</div><div class="modalBottomSpace"></div></div>`
}
...
function selectPlaylistSong(id) {
  ...
  if (state.playingId && String(state.playingId) !== key) {
    ...
    if (String(visualQueuedSongId || '') === key) {
...
function handleTouchEnd(e) {
  ...
  if (state.lyricsPanelOpen) {
    ...
    // Tela principal -> swipe para esquerda abre Markers.
    state.localMarkersMode = true
    state.playlistView = 'markers'
    postCommand('set_page', { page: 'playlist' })
    postCommand('set_parts_visibility', { page: 'playlist', visible: '1' })
...
function renderRows(items, type) {
  ...
  return items.map((item, index) => {
    ...
    const isQueued = type === 'song' && String(visualQueuedSongId || '') === itemId
...
function bindEvents() {
  document.querySelector('[data-action="go-playlist"]')?.addEventListener('click', openPlaylist)
  document.querySelector('[data-action="go-regions"]')?.addEventListener('click', openRegions)
  document.querySelector('[data-action="open-markers"]')?.addEventListener('click', openMarkersPanel)
  document.querySelector('[data-action="close-markers"]')?.addEventListener('click', closeMarkersPanel)
  ...
  document.querySelector('[data-action="toggle-settings"]')?.addEventListener('click', handleToggleSettingsMenu)
  ...
  document.querySelector('[data-action="recados-cancel"]')?.addEventListener('click', closeRecadosModal)
  ...
  bindModalCloseAction(document.querySelector('[data-action="close-mixer"]'), 'close-mixer', closeMixerModal)
  bindModalCloseAction(document.querySelector('[data-action="close-mixer-volume"]'), 'close-mixer-volume', closeMixerVolumeModal)
  ...
  document.querySelector('[data-action="play"]')?.addEventListener('click', handlePlayToggle)
  ...
  document.querySelector('[data-action="autoplay"]')?.addEventListener('click', handleAutoplayToggle)
  document.querySelector('[data-action="create-block"]')?.addEventListener('click', () => postCommand('create_block'))
  document.querySelector('[data-action="auto-bloco"]')?.addEventListener('click', handleAutoBlocoToggle)
  document.querySelector('[data-action="open-lyrics-panel"]')?.addEventListener('click', openLyricsPanel)
  ...
  document.querySelector('[data-action="loop"]')?.addEventListener('click', handleLoopToggle)
  ...
  document.querySelectorAll('[data-song-id]').forEach((el) => {
    ...
    el.addEventListener('click', () => selectPlaylistSong(id))
  })

  document.querySelectorAll('[data-marker-id]').forEach((el) => el.addEventListener('click', () => selectMarker(el.getAttribute('data-marker-id'))))
  ...
  document.querySelectorAll('.listBox, .lyricsScreen').forEach((swipeList) => {
    ...
    swipeList.addEventListener('touchend', handleTouchEnd, { passive: true })
  })
...
function renderBridgePopupHtml(extraClass = '') {
  ...
  const bridgePopupClassSuffix = popupErrorForRender ? 'Error' : (/loop/i.test(String(popupTextForRender || '')) ? 'Success' : 'Marker')
  const extra = extraClass ? ` ${extraClass}` : ''
  return `<div class="appPopup appPopup${bridgePopupClassSuffix}${extra} ${popupPersistentForRender ? 'appPopupPersistent' : 'appPopupTransient'} ${bridgePopupDisplay.fading ? 'appPopupHidden' : ''}">${escapeHtml(popupTextForRender)}</div>`
}
...
function render() {
  ...
  const content = state.activeTab === 'regions'
    ? `<div class="contentPanel"><div class="controlsStickyPanel"><div class="controlsRowRegions"><button class="${getPlayButtonClass()}" data-action="play">${getPlayButtonLabel()}</button><button class="btnLyricsOpen lyricsNavButton" data-action="open-lyrics-panel">&lt;&lt;</button></div><div class="sectionLabel sectionLabelSticky">MÚSICAS</div>${renderNowPlayingBanner()}</div><div class="listBox">${renderRows(state.regions, 'region')}</div></div>`
    : `<div class="contentPanel ${state.playlistView === 'markers' ? 'markerContentPanel' : ''}"><div class="controlsStickyPanel">${state.playlistView === 'markers'
        ? `<div class="controlsRowPlaylist"><button class="${getPlayButtonClass()}" data-action="play">${getPlayButtonLabel()}</button><button class="${state.loopActive ? 'btnLoopActive loopBlink' : 'btn'}" data-action="loop">Loop</button><button class="btnLyricsOpen lyricsNavButton markersInlineBackButton" data-action="close-markers">&lt;&lt;</button></div>`
        : `<div class="controlsRowPlaylist controlsRowPlaylistFour"><button class="${getPlayButtonClass()}" data-action="play">${getPlayButtonLabel()}</button><button class="${state.autoplayEnabled ? 'btnAutoplayActive' : 'btn'}" data-action="autoplay">AUTO</button><div></div></div>`}
       ${state.playlistView === 'markers' ? '' : `<div class="sectionLabel sectionLabelSticky">REPERTÓRIOS</div>`}${renderNowPlayingBanner()}</div>
