...
function render() {
  ...
  const showMarkerCancelFloating = isMarkersPanelOpen() && !!(String(getDirectorMarkerArmedVisualId?.() || '') || String(state.markerGoFlashId || '') || getActiveDirectorMarkerArmedLocalHoldId?.())
  const markerFooterHtml = state.playlistView === 'markers'
    ? `<div class="vshookFixedFooter">${showMarkerCancelFloating ? `<button class="floatingCancelButton markerCancelGlobalButton" data-action="marker-cancel">Cancelar</button>` : `<div class="footerButtonPlaceholder"></div>`}</div>`
    : ''
  const markerCancelGlobalButton = showMarkerCancelFloating ? `<button class="vshookMarkerCancelOverlay markerCancelGlobalButton" data-action="marker-cancel" type="button">Cancelar</button>` : ''
